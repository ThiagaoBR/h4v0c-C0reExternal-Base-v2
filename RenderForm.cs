...
namespace C0reExternalBase_v2
{
    public partial class RenderForm : Form
    {
        ...
        public Visuals _Visuals = new Visuals();
        ...
        public void RenderFrame()
        {
            try
            {
                ...
                if (m_pWindowHandle == m_pForeGroundWindow || Handle == m_pForeGroundWindow)
                {
                    ...
                    _Render.UpdateShift(-Left + m_WindowRectangle.X, -Top + m_WindowRectangle.Y);

                    // Render Menu
                    MenuRun(_Render);

                    // Render Radar Background
                    //if (Settings.m_bRadar)
                    //_Visuals.DrawRadar(m_WindowRectangle);
                    ...
                    for (var i = 0; i < 64; i++)
                    {
                        ...
                        // Renders An ESP-Box
                        //if (Settings.m_bESP)
                        _Visuals.DrawESPBox(i, color, m_WindowRectangle);

                        // Renders Entity On The Radar
                        //if (Settings.m_bRadar)
                        //_Visuals.DrawEntityOnRadar(i, color, m_WindowRectangle);

                        // Renders Snaplines
                        //if (Settings.m_bSnaplines)
                        _Visuals.DrawSnaplines(i, Color.FromArgb(255, 170, 170, 170), m_WindowRectangle);
                    }
                }
        ...
        private void PrepareRender()
        {
            ...
            _Screen = new Renderer(m_Device, -Left, -Top);
            _Render = new Renderer(m_Device);
        }

        private void PrepareMemory()
        {
            ManageMemory.Initialize("csgo");
            m_ClientPointer = ManageMemory.GetModuleAdress("client_panorama");
